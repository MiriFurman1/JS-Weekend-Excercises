const toWeirdCase=(str)=>{
    ...
    let newArr=word.split("") //! split the current word to letters
        for(let j=0;j<wordArr[i].length;j++){ //!loop over the letters
            if(j%2==0){ //!if the index is even
            newArr[j]=wordArr[i][j].toUpperCase() //!uppercase and save it to the letters array
            }
        }
        wordArr[i]=newArr.join("") //! save the finished word to the word array
    }
